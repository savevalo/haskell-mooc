...
longest :: Ord a => [[a]] -> [a]
longest = minimumBy compareLengthAndFirst
  where
    compareLengthAndFirst xs ys =
      if length xs == length ys
        then compare (head xs) (head ys)
...
incrementKey f xs = [(k, if f == k then v+1 else v) | (k, v) <- xs]
...
transfer from to amount bank
  ...
  | otherwise = Map.adjust (\i -> i - amount) from $ Map.adjust (\i -> i + amount) to bank
