...
oddFactorial 1 = 1
oddFactorial n = if n `mod` 2 == 0 then oddFactorial(n-1)
...
countdown n = "Ready! " ++ valiosa n "" ++ " Liftoff!"

valiosa :: Integer -> String -> String
valiosa 0 s = s
valiosa n s = valiosa(n-1) (s ++ show n ++ "... ")
...
smallestDivisor n = if (n `mod` 2) > 0 then apu n 2 else 2

apu :: Integer -> Integer -> Integer
apu x y = if (x `mod` y) == 0 then y else apu x (y+1)
