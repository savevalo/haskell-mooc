...
mapMaybe2 x (Just y) (Just z) = Just (x y z)
...
firstHalf xs = if (length xs `mod` 2) == 0 then take (length xs `div` 2) xs else take (fromIntegral (length xs `div` 2) + 1) xs
...
multiApp _ [] _ = []
multiApp f (g:gs) x = f x : [g x | g <- gs]
