...
totalPrice (MkShoppingEntry nimi hinta maara) = fromIntegral maara * hinta
...
setName name (MkPerson nimi age) = MkPerson name age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MkPerson name ika) = MkPerson name age
...
data UpDown = Laskeva Int | Nouseva Int
...
get :: UpDown -> Int
get (Nouseva arvo) = arvo
get (Laskeva arvo) = arvo

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (Nouseva arvo) = Nouseva (arvo + 1)
tick (Laskeva arvo) = Laskeva (arvo - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Nouseva arvo) = Laskeva arvo
toggle (Laskeva arvo) = Nouseva arvo
...
toList Empty = []
toList (Pair k v muut) = (k, v): toList muut
